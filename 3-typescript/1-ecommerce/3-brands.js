...
var read_json_util_1 = require("./utils/read-json.util");
function getCountriesWithBrandsAndProductCount(brands, products) {
    return __awaiter(this, void 0, void 0, function () {
        var brandCountriesMap, result;
        return __generator(this, function (_a) {
            brandCountriesMap = new Map();
            brands.forEach(function (brand) {
                var splittedHeadquarters = brand.headquarters.split(",");
                var countryName = splittedHeadquarters[splittedHeadquarters.length - 1].trim();
                brandCountriesMap.set(Number(brand.id), countryName);
            });
            result = products.reduce(function (counter, product) {
                var country = brandCountriesMap.get(product.brandId);
                if (country) {
...
Promise.all([
    ...
    .then(function (_a) {
    var products = _a[0], brands = _a[1];
    var result = getCountriesWithBrandsAndProductCount(brands, products);
    return result;
})
